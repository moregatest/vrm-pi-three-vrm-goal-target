...
預設 **`hybrid`**(幾何身體:手臂/腿**位置**+面向正確 + **Kalidokit 手指**;對實拍最穩)。`--engine simple` 是無手指版;`--engine kalidokit` 有手臂 twist 但在本專案手搭骨架上手臂會舉高(座標慣例不符)。**務必跟原片 contact sheet 比對**再調(別只看 avatar)。常用 flags:
...
## 8. GPU 高品質路線(Colab)— 含手臂 twist

本機(M4 無 CUDA)免費路線只能做到「位置正確、無 twist」。要更高品質(肢體 twist、較穩的深度)需 GPU,走雲端:

1. 開 `colab/dance_to_smpl.ipynb`(Colab,Runtime 設 **GPU/T4**)→ 上傳影片 → 跑 **ROMP** → 下載 `smpl.json`(每幀 SMPL 72 維 pose)。
2. 本機轉檔 + 預覽:
   ```bash
   node tools/smpl-to-vrma.mjs smpl.json public/vrma/dance.vrma --face-flip
   node tools/render-vrma-preview.mjs --vrma public/vrma/dance.vrma --vrm /avatars/default.vrm --out dance.mp4 --contact-sheet
   ```
3. 看 contact sheet 跟原片比對,調 `smpl-to-vrma` 的座標 flags(`--flip-x/-y/-z`、`--face-flip`、`--smooth`)到對齊 —— SMPL→VRM 的座標慣例通常要 1 輪微調。

`smpl-to-vrma.mjs` 吃的格式:`{ "fps":30, "frames":[ {"thetas":[72 個 float], "trans":[x,y,z]}, ... ] }`。
想要再更好(world-grounded、抑腳滑)可在 Colab 改用 **WHAM/GVHMR**,輸出成同樣的 `smpl.json` 即可共用同一支轉檔(見 notebook 末的 Advanced)。
